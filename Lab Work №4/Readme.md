...
3. **[GET] /Account/GetBasisSum**: запрос пустой, через Auth передаётся токен, в ответе базисная сумма.
...
5. **[PUT] /Account/Update**: параметром передаётся новая базисная сумма, через Auth передаётся токен, в ответе - id и новая сумма.
...
3. **[GET] /AccountEntry/GetAll**: запрос пустой, через Auth передаётся токен, в ответе список из всех записей в кошельке.
**Параметры**: отсутствуют
**Ответы**: 200 (успех), 401 (для неавторизованных), 403 (при несуществовании кошелька)
...
5. **[PUT] /AccountEntry/Update**: параметром передаётся ID записи, через Auth передаётся токен, в ответе - id, дата, сумма, id категории, id аккаунта.
...
7. **[DELETE] /AccountEntry/Delete**: параметром передаётся ID записи, через Auth передаётся токен, в ответе - ничего.
**Параметры**: accountEntryId
**Ответы**: 200 (успех), 401 (для неавторизованных), 403 (при несуществовании кошелька)
